...
UPDATE 2026-09-25 (root-cause row): the TT-LEDGER allocator instrument (temporary, TT_LEDGER-gated, on the pinned tree's build_instr) NAMES the structure: one (16,777,216 B + 3,932,160 B) pair of `ttnn::to_layout` tilize outputs leaked per GDN layer (48 layers) per request, on each request's FIRST decode step, in the GDN state-scatter web (GdnStateScatterKernel -> ScatterRowsExact/ScatterRowsDevice, tenstorrent_gdn.cpp:1004-1086) — 48x(16+3.75) MiB = 948 MiB/request, the ledger's -949.3 MB/request exactly. The buffers are the web's unregistered transients (the per-request re-staged state shadow the commit replaces, and the not-resident rows upload); their scope-exit free is skipped because ttnn's Tensor::from_vector -> to_device -> to_layout(TILE) upload chain leaves their attributes referenced. The program cache is EXONERATED (request 3 leaks its full step-0 set at ZERO PC-INSERTs; the leg is fully eager — no capture cycles exist). Fix: the keepquant web's force-reclaim discipline applied to the scatter's dead transients (tenstorrent_gdn.cpp, TTReclaimPlanes after the commit). Evidence: docs/bench-evidence/tt-ttm-retention-rootcause-20260925.md + the upstream reproducer package docs/bench-evidence/tt-ttm-retention-repro-20260925/.
